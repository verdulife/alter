...
// TimeContext carries the clock and timezone that time resolution and
// recurrence building need. Both values are provided by the caller (the
// application layers), never by the LLM, so the resolution functions stay
// pure and deterministic.
type TimeContext struct {
	// Now is the current time. Used for relative resolution and for deciding
	// "today" vs "tomorrow" when an absolute time has passed.
	Now time.Time
	// Timezone is the user's timezone. Resolved from ALTER_TIMEZONE or time.Local.
	Timezone *time.Location
}

// OneShotSpec describes when a one-shot reminder should fire, in the
// natural-language form: a relative Go duration string, or a local wall-clock
// time plus an optional date expression. It is the pure input needed to turn
// a reminder request into a concrete fire time.
...
// (B3 S4). It carries ONLY what the interpreter extracted from the user
// message; Go derives timezone, interval default, anchor year and validates
// the resulting spec. Nil pointers and empty slices mean "not provided"
// (never a zero value).
type RecurrenceParams struct {
	...
	// year from the current date (TimeContext.Now).
...
// ResolveTime converts a OneShotSpec into a concrete time.Time using the
...
func ResolveTime(spec OneShotSpec, ctx TimeContext) (time.Time, error) {
	if spec.Relative != "" {
...
func resolveAbsolute(timeStr, dateStr string, ctx TimeContext) (time.Time, error) {
...
// canonical RecurrenceSpec JSON stored in Trigger.Value (B3 S4). It is the
// application side of the contract:
//   - timezone always comes from TimeContext (the user's), never from the LLM;
...
func BuildRecurrenceJSON(p RecurrenceParams, ctx TimeContext) (string, error) {
	if p.Freq == "" {
...
// ALWAYS completed by the application from TimeContext.Now; the interpreter
// never provides them).
func resolveAnchor(p RecurrenceParams, weekdaysMask int, ctx TimeContext) (string, error) {
	now := ctx.Now.In(ctx.Timezone)
...
// the domain's bit encoding (Monday=1<<0 .. Sunday=1<<6): the natural-language
// contract maps 1=monday..7=sunday onto exactly those bits, so the two layers
// stay compatible without duplicating any calendar rule.
